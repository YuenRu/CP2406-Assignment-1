...
public class Car {
    ...
    public void move() {
        ...
        if (carX == road.RoadList.get(i + 1).x
            ...
            if (!judgeInflexion()) { //Determine whether the turning point of the U-turn is reached, and if it is not the turning point, then judge the traffic lights
                if (judgeLight(s1) == false) {//Judgment traffic light
                    moveLength = 0;
                }
            }
    ...
    //Walk in different directions, and judge the status of the previous car before walking
    public void differentDirection() {
        if (y > 0) {
            //Before you go, first judge the previous car in that direction. If the status of the previous car is stopped, stop; otherwise, go forward.
    ...
    //To determine the traffic light
    ...
    //Determine whether it is the turning point of the U-turn.
